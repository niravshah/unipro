    ...
    <div class="row">
      <div class="col-md-12">
        <vue-tabs @tab-change="tabChanged">
          ...
          <v-tab class="u-p-medium" v-for="item in items" :title="'' + item.catalogue_id" :key="item.catalogue_id">

            <div align="right" class="u-mt-small">
              <button v-if="!edit" @click="editForm(item)" class="c-btn c-btn--small">
                Edit {{item.catalogue_id}}
              </button>
              <button v-if="!edit" @click="deleteForm(item)" class="c-btn c-btn--small">
                Delete {{item.catalogue_id}}
              </button>
              <button v-if="edit" @click="saveForm(item)" class="c-btn c-btn--small c-btn--success">
                Save {{item.catalogue_id}}
              </button>
              <button v-if="edit" @click="cancelEdit(item.catalogue_id)" class="c-btn c-btn--small c-btn--secondary">
                Cancel
              </button>
            </div>
            <div class="row u-mt-small">
              <div class="col-md-3">
                <div class="c-field u-mb-small">
                  ...
                  <p id="location_id">{{item.catalogue_id}}</p>
                </div>
              </div>
            </div>
            <div class="row u-mt-small">
  ...
  export default {
    ...
    methods: {
      ...
      deleteForm: async function (item) {
        Service.delete(item.catalogue_id).then(resp => {
      ...
      saveForm: function (item) {
        this.edit = false;
        Service.update(item.catalogue_id, this.upd).then(resp => {
          ToastedService.showInfo("Catalogue Item Updated", 2000);
          this.getData();
        }).catch(err => {
          if (err.response) {
            ToastedService.showError("Could not update this Item." + err.response.data.message, 2000)
          } else {
            ToastedService.showError("Could not update this Item." + err.message, 2000)
          }
        })
      }
    }
